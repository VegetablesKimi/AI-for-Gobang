import copy
import time
from enum import IntEnum
import pygame
from pygame.locals import *

time = time.strftime("%Y-%m-%d %H:%M:%S")
version = str(time)

# basic parameters
square_size = 40 # the size of a single square
chess_size = square_size // 2 - 2 # size of the chess
web_board = 15 # number of square in the board (nxn)
map_w = web_board * square_size  # length of the chess board
map_h = web_board * square_size  # height of the chess board
info_w = 60 # the width of the button surface
button_w = 120 # the width of the button
button_h = 45 # the height of the button
screen_w = map_w # the width of the whole screen
screen_h = map_h + info_w # the height of the whole screen\


class MAP_ENUM(IntEnum): 
    """
    Enumarate the situation of the map cell
    """
    be_empty = 0, # nobody here
    player1 = 1, # player1, white chess
    player2 = 2, # player2, black chess
    out_of_range = 3, # out of range


class Map: # define a map class
    def __init__(self, width, height): 
        """
        Constructor function
        :param width: the width of the map
        :param height: the height of the map
        :return:
        """
        self.width = width
        self.height = height
        self.map = [[0 for x in range(self.width)] for y in range(self.height)]
        self.steps = [] # record the sequence of the steps
    
    def get_init(self): 
        """
        Initialize the chess board
        """
        for y in range(self.height):
            for x in range(self.width):
                self.map[y][x] = 0
        self.steps = []
    
    def intoNextTurn(self, turn): 
        """
        Go to the next turn and change the player
        :param turn: the player
        :return: the player in the next turn
        """
        if turn == MAP_ENUM.player1:
            return MAP_ENUM.player2
        else:
            return MAP_ENUM.player1
    
    def getLocate(self, x, y):  
        """
        Get the index and return the exact location
        :param x: the x coordinate
        :param y: the y coordinate
        :return: the index and the exact location
        """
        map_x = x * square_size
        map_y = y * square_size
        return (map_x, map_y, square_size, square_size)  

    def getIndex(self, map_x, map_y):  
        """
        Get the location and return the index
        :param map_x: the x exact location
        :param map_y: the y exact location
        :return: the x and y index 
        """
        x = map_x // square_size
        y = map_y // square_size
        return (x, y)
    
    def isInside(self, map_x, map_y): 
        """
        Judge whether the chess is in the correct range
        :param map_x: exact location of x
        :param map_y: exact location of y
        :return: True if the chess is inside the correct range
        """
        if (map_x <= 0 or map_x >= map_w or map_y <= 0 or map_y >= map_h):
            return False
        return True
    
    def isEmpty(self, x, y): 
        """
        Check if the chess cell is empty
        :param x: the x coordinate of the chess cell
        :param y: the y coordinate of the chess cell
        :return: True if the chess cell is empty
        """
        return (self.map[y][x] == 0)
    
    def click(self, x, y, type): 
        """
        Perform the chess action click and record the action in the steps
        :param x: the x coordinate of the desired chess cell
        :param y: the y coordinate of the desired chess cell
        :param type: the type of the player
        """
        self.map[y][x] = type.value
        self.steps.append((x, y))
    
    def printChessPiece(self, screen): 
        """
        Draw the chess 
        :param screen: the screen to show the chess
        """
        player_1 = (255, 245, 238)
        player_2 = (41, 36, 33)
        player_colour = [player_1, player_2]
        for i in range(len(self.steps)):
            x, y = self.steps[i]
            map_x, map_y, width, height = self.getLocate(x, y)
            pos, radius = (map_x + width // 2, map_y + height // 2), chess_size
            turn = self.map[y][x]
            pygame.draw.circle(screen, player_colour[turn - 1], pos, radius)
    
    def drawBoard(self, screen): 
        """
        Draw the chess board
        :param screen: the screen to show the chess board
        """
        colour = (0, 0, 0)
        for y in range(self.height): # draw the horizontal lines
            start_pos = (square_size // 2, square_size // 2 + square_size * y)
            end_pos = (map_w - square_size // 2, square_size // 2 + square_size * y)
            pygame.draw.line(screen, colour, start_pos, end_pos, 1)
        for x in range(self.width): # draw the vertical lines
            start_pos = (square_size // 2 + square_size * x, square_size // 2)
            end_pos = (square_size // 2 + square_size * x, map_h - square_size // 2)
            pygame.draw.line(screen, colour, start_pos, end_pos, 1)
    
# AI modules

class SITUATION(IntEnum):
    NONE = 0, # 无(nothing)
    SLEEP_TWO = 1, # 眠二(sleep two)
    LIVE_TWO = 2, # 活二(live two)
    SLEEP_THREE = 3, # 眠三(sleep three)
    LIVE_THREE = 4, # 活三(live three)
    CHONG_FOUR = 5,  # 冲四(rushin four)
    LIVE_FOUR = 6,  # 活四(live four)
    LIVE_FIVE = 7,  # 活五(live five)

SITUATION_NUM = 8 # the number of the situations

# for the enumeration of the situations
FIVE = SITUATION.LIVE_FIVE.value
L4, L3, L2 = SITUATION.LIVE_FOUR.value, SITUATION.LIVE_THREE.value, SITUATION.LIVE_TWO.value
S4, S3, S2 = SITUATION.CHONG_FOUR.value, SITUATION.SLEEP_THREE.value, SITUATION.SLEEP_TWO.value


class MyAI(): # define the MyAI class
    def __init__(self, chess_len): # constructor function
        self.len = chess_len # current size of the board

        # record for each cell: left, right, left diagonal, right doagonal
        self.record = [[[0, 0, 0, 0] for i in range(chess_len)] for j in range(chess_len)]

        # record for current number of the chess type 
        self.count = [[0 for i in range(SITUATION_NUM)] for j in range(2)]

        # point for the position (higher if closer to the center under same situation) 
        self.position_isgreat = [
            [(web_board - max(abs(i - web_board / 2 + 1), abs(j - web_board / 2 + 1))) for i in range(chess_len)]
            for j in range(chess_len)]
    
    def get_init(self): 
        """
        Initialize the record and count
        """
        for i in range(self.len):
            for j in range(self.len):
                for k in range(4):
                    self.record[i][j][k] = 0
        for i in range(len(self.count)):
            for j in range(len(self.count[0])):
                self.count[i][j] = 0
        self.save_count = 0
    
    def iswin(self, board, turn): 
        """
        Judge if the present person wins
        :param board: the current board
        :param turn: the current player
        """
        return self.evaluate(board, turn, True)
    
    def get_no_move(self, board, turn): 
        """
        Get all the coordinates of the empty cells, listing from good position to bad position
        :param board: the current board
        :param turn: the current player
        """
        moves = []
        for y in range(self.len):
            for x in range(self.len):
                if board[y][x] == 0:
                    score = self.position_isgreat[y][x]
                    moves.append((score, x, y))
        moves.sort(reverse=True) 
        return moves
    
    def search(self, board, turn): 
        """
        Find the best move under the current situation
        :param board: the current board
        ::param turn: the current player
        """
        moves = self.get_no_move(board, turn)
        best_move = None
        max_score = -99999
        for score, x, y in moves:
            board[y][x] = turn.value
            score = self.evaluate(board, turn)
            board[y][x] = 0
            if score > max_score:
                max_score = score
                best_move = (max_score, x, y)
        return best_move
    
    def findBestChess(self, board, turn):
        # time1 = time.time()
        score, x, y = self.search(board, turn)
        # time2 = time.time()
        # print('time:%f  (%d, %d)' % ((time2 - time1), x, y))
        return (x, y)
    
    def getScore(self, mychess, yourchess):
        """
        get the score for each point, by listing all the possible chess type
        """
        mscore, oscore = 0, 0
        if mychess[FIVE] > 0:
            return (10000, 0)
        if yourchess[FIVE] > 0:
            return (0, 10000)
        if mychess[S4] >= 2:
            mychess[L4] += 1
        if yourchess[L4] > 0:
            return (0, 9050)
        if yourchess[S4] > 0:
            return (0, 9040)
        if mychess[L4] > 0:
            return (9030, 0)
        if mychess[S4] > 0 and mychess[L3] > 0:
            return (9020, 0)
        if yourchess[L3] > 0 and mychess[S4] == 0:
            return (0, 9010)
        if (mychess[L3] > 1 and yourchess[L3] == 0 and yourchess[S3] == 0):
            return (9000, 0)
        if mychess[S4] > 0:
            mscore += 2000
        if mychess[L3] > 1:
            mscore += 500
        elif mychess[L3] > 0:
            mscore += 100
        if yourchess[L3] > 1:
            oscore += 2000
        elif yourchess[L3] > 0:
            oscore += 400
        if mychess[S3] > 0:
            mscore += mychess[S3] * 10
        if yourchess[S3] > 0:
            oscore += yourchess[S3] * 10
        if mychess[L2] > 0:
            mscore += mychess[L2] * 4
        if yourchess[L2] > 0:
            oscore += yourchess[L2] * 4
        if mychess[S2] > 0:
            mscore += mychess[S2] * 4
        if yourchess[S2] > 0:
            oscore += yourchess[S2] * 4
        return (mscore, oscore) # my score, other score
    
    def evaluate(self, board, turn, checkWin=False):
        """
        Evaluate the score based on the above getScore function
        :param board: the current board
        :param turn: the current player
        :param checkWin: if checkWin is True, then return True if the current player wins; False for default
        """
        self.get_init()
        if turn == MAP_ENUM.player1:
            me = 1 # player 1, white chess
            you = 2 # player 2, black chess
        else:
            me = 2 # player 2, black chess
            you = 1 # player 1, white chess
        for y in range(self.len):
            for x in range(self.len):
                if board[y][x] == me:
                    self.evaluatePoint(board, x, y, me, you)
                elif board[y][x] == you:
                    self.evaluatePoint(board, x, y, you, me)
        mychess = self.count[me - 1]
        yourchess = self.count[you - 1]
        if checkWin:
            return mychess[FIVE] > 0  # check if the current player wins
        else:
            mscore, oscore = self.getScore(mychess, yourchess)
            return (mscore - oscore)

    def evaluatePoint(self, board, x, y, me, you):
        """
        Evaluate the score of each point
        :param board: the current board
        :param x: the x coordinate of the point
        :param y: the y coordinate of the point
        :param me: the current player
        :param you: the other player
        """
        direction = [(1, 0), (0, 1), (1, 1), (1, -1)]  # define four direction
        for i in range(4):
            if self.record[y][x][i] == 0:
                # check the chess type for the current direction
                self.getBasicSituation(board, x, y, i, direction[i], me, you, self.count[me - 1])
            else:
                self.save_count += 1

    def getLine(self, board, x, y, direction, me, you):
        """
        Store the chess type of the direction for later use
        :param board: the current board
        :param x: the x coordinate of the point
        :param y: the y coordinate of the point
        :param direction: the direction represented by (1, 0) or (0, 1) or (1, 1) or (1, -1)
        :param me: the current player
        :param you: the other player
        :return: the chess type of the direction
        """
        line = [0 for i in range(9)]
        tmp_x = x + (-5 * direction[0])
        tmp_y = y + (-5 * direction[1])
        for i in range(9):
            tmp_x += direction[0]
            tmp_y += direction[1]
            if (tmp_x < 0 or tmp_x >= self.len or tmp_y < 0 or tmp_y >= self.len):
                line[i] = you  # out of the board
            else:
                line[i] = board[tmp_y][tmp_x]
        return line

    def getBasicSituation(self, board, x, y, dir_index, dir, me, you, count):
        """
        Identify the current direction of play as a specific situation (such as identifying MMMMX as a RUSH FOUR)
        :param board: the current board
        :param x: the x coordinate of the point
        :param y: the y coordinate of the point
        :param dir_index: the index of the direction
        :param dir: the direction represented by (1, 0) or (0, 1) or (1, 1) or (1, -1)
        :param me: the current player
        :param you: the other player
        :param count: the number of chess for the current player
        """
        def setRecord(self, x, y, left, right, dir_index, direction):
            tmp_x = x + (-5 + left) * direction[0]
            tmp_y = y + (-5 + left) * direction[1]
            for i in range(left, right):
                tmp_x += direction[0]
                tmp_y += direction[1]
                self.record[tmp_y][tmp_x][dir_index] = 1

        empty = MAP_ENUM.be_empty.value
        left_index, right_index = 4, 4
        line = self.getLine(board, x, y, dir, me, you)
        while right_index < 8:
            if line[right_index + 1] != me:
                break
            right_index += 1
        while left_index > 0:
            if line[left_index - 1] != me:
                break
            left_index -= 1
        left_range, right_range = left_index, right_index
        while right_range < 8:
            if line[right_range + 1] == you:
                break
            right_range += 1
        while left_range > 0:
            if line[left_range - 1] == you:
                break
            left_range -= 1
        chess_range = right_range - left_range + 1
        if chess_range < 5:
            setRecord(self, x, y, left_range, right_range, dir_index, dir)
            return SITUATION.NONE
        setRecord(self, x, y, left_index, right_index, dir_index, dir)
        m_range = right_index - left_index + 1
        if m_range == 5:
            count[FIVE] += 1
        # LIVE_FOUR & RUSH_FOUR
        if m_range == 4:
            left_empty = right_empty = False
            if line[left_index - 1] == empty:
                left_empty = True
            if line[right_index + 1] == empty:
                right_empty = True
            if left_empty and right_empty:
                count[L4] += 1
            elif left_empty or right_empty:
                count[S4] += 1
        # LIVE_THREE & SLEEP_THREE
        if m_range == 3:
            left_empty = right_empty = False
            left_four = right_four = False
            if line[left_index - 1] == empty:
                if line[left_index - 2] == me:  # MXMMM
                    setRecord(self, x, y, left_index - 2, left_index - 1, dir_index, dir)
                    count[S4] += 1
                    left_four = True
                left_empty = True
            if line[right_index + 1] == empty:
                if line[right_index + 2] == me:  # MMMXM
                    setRecord(self, x, y, right_index + 1, right_index + 2, dir_index, dir)
                    count[S4] += 1
                    right_four = True
                right_empty = True
            if left_four or right_four:
                pass
            elif left_empty and right_empty:
                if chess_range > 5:  # XMMMXX, XXMMMX
                    count[L3] += 1
                else:  # PXMMMXP
                    count[S3] += 1
            elif left_empty or right_empty:  # PMMMX, XMMMP
                count[S3] += 1
        # LIVE_TWO & SLEEP_TWO
        if m_range == 2:
            left_empty = right_empty = False
            left_three = right_three = False
            if line[left_index - 1] == empty:
                if line[left_index - 2] == me:
                    setRecord(self, x, y, left_index - 2, left_index - 1, dir_index, dir)
                    if line[left_index - 3] == empty:
                        if line[right_index + 1] == empty:  # XMXMMX
                            count[L3] += 1
                        else:  # XMXMMP
                            count[S3] += 1
                        left_three = True
                    elif line[left_index - 3] == you:  # PMXMMX
                        if line[right_index + 1] == empty:
                            count[S3] += 1
                            left_three = True
                left_empty = True
            if line[right_index + 1] == empty:
                if line[right_index + 2] == me:
                    if line[right_index + 3] == me:  # MMXMM
                        setRecord(self, x, y, right_index + 1, right_index + 2, dir_index, dir)
                        count[S4] += 1
                        right_three = True
                    elif line[right_index + 3] == empty:
                        # setRecord(self, x, y, right_index+1, right_index+2, dir_index, dir)
                        if left_empty:  # XMMXMX
                            count[L3] += 1
                        else:  # PMMXMX
                            count[S3] += 1
                        right_three = True
                    elif left_empty:  # XMMXMP
                        count[S3] += 1
                        right_three = True
                right_empty = True
            if left_three or right_three:
                pass
            elif left_empty and right_empty:  # XMMX
                count[L2] += 1
            elif left_empty or right_empty:  # PMMX, XMMP
                count[S2] += 1
        # Special LIVE_TWO & SLEEP_TWO (with space)
        if m_range == 1:
            left_empty = right_empty = False
            if line[left_index - 1] == empty:
                if line[left_index - 2] == me:
                    if line[left_index - 3] == empty:
                        if line[right_index + 1] == you:  # XMXMP
                            count[S2] += 1
                left_empty = True
            if line[right_index + 1] == empty:
                if line[right_index + 2] == me:
                    if line[right_index + 3] == empty:
                        if left_empty:  # XMXMX
                            count[L2] += 1
                        else:  # PMXMX
                            count[S2] += 1
                elif line[right_index + 2] == empty:
                    if line[right_index + 3] == me and line[right_index + 4] == empty:  # XMXXMX
                        count[L2] += 1
        # none chess type if not belongs to anyone above
        return SITUATION.NONE



    
class Button: 
    """
    Control process button class (parent class)
    """
    def __init__(self, screen, text, x, y, color, enable):  # constructor function
        self.screen = screen
        self.width = button_w
        self.height = button_h
        self.button_color = color
        self.text_color = (255, 255, 255)  # pure white
        self.enable = enable
        self.font = pygame.font.SysFont(None, button_h * 2 // 3)
        self.rect = pygame.Rect(0, 0, self.width, self.height)
        self.rect.topleft = (x, y)
        self.text = text
        self.init_msg()

    def init_msg(self):
        """
        Initialize the button
        """
        if self.enable:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[0])
        else:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
        self.msg_image_rect = self.msg_image.get_rect()
        self.msg_image_rect.center = self.rect.center
    
    def draw(self):
        """
        Fill the button according to the enable state of the button, the specific color is controlled in the subsequent subclass
        """
        if self.enable:
            self.screen.fill(self.button_color[0], self.rect)
        else:
            self.screen.fill(self.button_color[1], self.rect)
        self.screen.blit(self.msg_image, self.msg_image_rect)


class WhiteStartButton(Button):  # start button class (choose white chess)
    def __init__(self, screen, text, x, y):  # constructor function
        super().__init__(screen, text, x, y, [(26, 173, 25), (158, 217, 157)], True)

    def click(self, game):  # click button
        if self.enable:  # start the game and get initialized
            game.start()
            game.winner = None
            game.multiple = False
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False

    def unclick(self):  # unclick button
        if not self.enable:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[0])
            self.enable = True


class BlackStartButton(Button):  # start button class (choose black chess)
    def __init__(self, screen, text, x, y):  # cinstructor function
        super().__init__(screen, text, x, y, [(26, 173, 25), (158, 217, 157)], True)

    def click(self, game):  # click button
        if self.enable:  # start the game and get initialized (AI first)
            game.start()
            game.winner = None
            game.multiple = False
            game.useAI = True
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False

    def unclick(self):  # unclick button
        if not self.enable:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[0])
            self.enable = True


class GiveupButton(Button):  
    """
    Give up button, you can use it under any circumstance
    """
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(230, 67, 64), (236, 139, 137)], False)

    def click(self, game):  # end the game and judge the winner
        """
        End the game and determine the winner
        """
        if self.enable:
            game.is_play = False
            if game.winner is None:
                game.winner = game.map.intoNextTurn(game.player)
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False

    def unclick(self):  # remain unchanged when unclicked
        if not self.enable:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[0])
            self.enable = True


class MultiStartButton(Button):  # start button (two players)
    def __init__(self, screen, text, x, y):  # constructor function
        super().__init__(screen, text, x, y, [(153, 51, 250), (221, 160, 221)], True)  # find your favourite color (purple)

    def click(self, game):  # click button
        if self.enable:  # start the game and get initialized
            game.start()
            game.winner = None
            game.multiple=True
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False

    def unclick(self):  # unclick button
        if not self.enable:
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[0])
            self.enable = True


class Game:  # pygame class
    def __init__(self, caption):
        pygame.init()
        self.screen = pygame.display.set_mode([screen_w, screen_h])
        pygame.display.set_caption(caption)
        self.clock = pygame.time.Clock()
        self.buttons = []
        self.buttons.append(WhiteStartButton(self.screen, 'choose white', 20, map_h))
        self.buttons.append(BlackStartButton(self.screen, 'choose black', 190, map_h))
        self.buttons.append(GiveupButton(self.screen, 'surrdender', 360, map_h))
        self.buttons.append(MultiStartButton(self.screen, 'Multiple', 530, map_h))
        self.is_play = False
        self.map = Map(web_board, web_board)
        self.player = MAP_ENUM.player1
        self.action = None
        self.AI = MyAI(web_board)
        self.useAI = False
        self.winner = None
        self.multiple = False

    def start(self):
        self.is_play = True
        self.player = MAP_ENUM.player1  # white first
        self.map.get_init()

    def play(self):
        """
        Draw the board
        """
        self.clock.tick(60)
        wood_color = (210, 180, 140)
        pygame.draw.rect(self.screen, wood_color, pygame.Rect(0, 0, map_w, screen_h))
        pygame.draw.rect(self.screen, (255, 255, 255), pygame.Rect(map_w, 0, info_w, screen_h))
        # draw the button
        for button in self.buttons:
            button.draw()
        if self.is_play and not self.isOver():
            if self.useAI and not self.multiple:
                x, y = self.AI.findBestChess(self.map.map, self.player)
                self.checkClick(x, y, True)
                self.useAI = False
            if self.action is not None:
                self.checkClick(self.action[0], self.action[1])
                self.action = None
            if not self.isOver():
                self.changeMouseShow()
        if self.isOver():
            self.showWinner()
            # self.buttons[0].enable = True
            # self.buttons[1].enable = True
            # self.buttons[2].enable = False
        self.map.drawBoard(self.screen)
        self.map.printChessPiece(self.screen)

    def changeMouseShow(self):  # change the mouse click into the expected chess when starting the game
        map_x, map_y = pygame.mouse.get_pos()
        x, y = self.map.getIndex(map_x, map_y)
        if self.map.isInside(map_x, map_y) and self.map.isEmpty(x, y):  # in the chess board and no chess in the cell
            pygame.mouse.set_visible(False)
            smoke_blue = (176, 224, 230)
            pos, radius = (map_x, map_y), chess_size
            pygame.draw.circle(self.screen, smoke_blue, pos, radius)
        else:
            pygame.mouse.set_visible(True)

    def checkClick(self, x, y, isAI=False):  # consequent action
        self.map.click(x, y, self.player)
        if self.AI.iswin(self.map.map, self.player):
            self.winner = self.player
            self.click_button(self.buttons[2])
        else:
            self.player = self.map.intoNextTurn(self.player)
            if not isAI:
                self.useAI = True

    def mouseClick(self, map_x, map_y):  # the play chess action
        if self.is_play and self.map.isInside(map_x, map_y) and not self.isOver():
            x, y = self.map.getIndex(map_x, map_y)
            if self.map.isEmpty(x, y):
                self.action = (x, y)

    def isOver(self):  # stop condition
        return self.winner is not None

    def showWinner(self):  # print the winner
        def showFont(screen, text, location_x, locaiton_y, height):
            font = pygame.font.SysFont(None, height)
            font_image = font.render(text, True, (255, 215, 0), (255, 255, 255))  # find your favourite color (golden yellow)
            font_image_rect = font_image.get_rect()
            font_image_rect.x = location_x
            font_image_rect.y = locaiton_y
            screen.blit(font_image, font_image_rect)

        if self.winner == MAP_ENUM.player1:
            str = 'White Wins!'
        else:
            str = 'Black Wins!'
        showFont(self.screen, str, map_w / 5, screen_h / 8, 100)  # top center, font size 100
        pygame.mouse.set_visible(True)

    def click_button(self, button):
        if button.click(self):
            for tmp in self.buttons:
                if tmp != button:
                    tmp.unclick()

    def check_buttons(self, mouse_x, mouse_y):
        for button in self.buttons:
            if button.rect.collidepoint(mouse_x, mouse_y):
                self.click_button(button)
                break


# example code from pygame1.9 help document
if __name__ == '__main__':
    game = Game(version)
    while True:
        game.play()
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                exit()
            elif event.type == pygame.MOUSEBUTTONDOWN:
                mouse_x, mouse_y = pygame.mouse.get_pos()
                game.mouseClick(mouse_x, mouse_y)
                game.check_buttons(mouse_x, mouse_y)


        
